...
import { Feature, TeamMember, WorkItemStatus } from '@/lib/types';
...
export function FeatureForm({
  ...
  const [status, setStatus] = useState<WorkItemStatus>(feature?.status || 'Not Started');
  const [assignedMembers, setAssignedMembers] = useState<string[]>(feature?.assignedMembers || []);
  const [sprintNumber, setSprintNumber] = useState<string>(feature?.sprintNumber?.toString() || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      isExpanded: feature?.isExpanded ?? true,
    });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={mode === 'create' ? 'New Feature' : 'Edit Feature'} size="lg">
